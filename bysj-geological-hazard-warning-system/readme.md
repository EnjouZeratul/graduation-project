...
- `docs/THESIS_REFERENCE.md`：毕业设计论文参考模板（章节结构、写作要点、图表建议、与代码文件对应关系）。
...
- `docs/`：技术文档、产品文档、论文参考文档。
- `tools/`：排错脚本与辅助工具脚本。
- `tianqi.com的模式/`：天气网页快照样本（解析/映射调试用）。
- `wunderground.com的模式/`：WU 页面快照样本（key 发现与解析调试用）。
- `国家气象数据API调用说明/`：国家气象 API 资料目录（站点表、字段说明等）。
- `备份合集/`：历史备份目录（不参与当前运行链路）。
